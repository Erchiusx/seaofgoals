...
.PHONY: lint test test-experiment-config analyze-trace plot-goal-coverage plot-goal-phases compile-skill compile-skill-with-preload-planner compile-skill-ordered-speculative compile-skill-bootstrap compile-skill-bootstrap-with-preload-planner build-workflows smoke-containerd smoke-bwrap run-experiment-config show-experiment-config experiment experiment-concurrent experiment-codex experiment-codex-fuse experiment-codex-fuse-preload experiment-pi experiment-pi-concurrent experiment-pi-concurrent-planner experiment-pi-fuse-concurrent-planner experiment-pi-fuse-ordered-speculative experiment-no-scfg experiment-nextjs-performance experiment-database-migrations experiment-mysql2postgres experiment-test-with-postgres experiment-docker-development experiments experiments-no-scfg view-trace serve-traces
...
compile-skill-ordered-speculative:
	test -n "$(SKILL_PATH)"
	SOG_COMPILER_STRATEGY=ordered-speculative cabal run exe:SeaOfGoals-compiler -- "$(SKILL_PATH)" "$(COMPILED_GOALS_OUT)" "$(SKILL_NAME)"
...
experiment-pi-fuse-ordered-speculative:
	test -n "$(SKILL_EXPERIMENT)"
	test -n "$(SOG_GRAPH_PATH)"
	SOG_AGENT_RUNNER=pi SOG_EXPERIMENT_DRIVER=host SOG_SCHEDULER=concurrent SOG_CONCURRENT_WORKSPACE=fuse SOG_CABAL_FLAGS="-f fuse" SOG_SPECULATIVE_ORDERED=1 SOG_INCREMENTAL_PLANNER=0 SOG_PRELOAD_GOAL_CONTEXT=0 SOG_PI_HISTORY_HANDOFF=0 SOG_HARNESS_LIFECYCLE=0 bash test-suite/skill-experiments/run-experiment.sh "$(SKILL_EXPERIMENT)"
