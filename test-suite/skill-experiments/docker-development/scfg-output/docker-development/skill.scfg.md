...
#### Step N001. Analyze current state
...
After this step, continue to any of Steps `N002` if necessary.

#### Step N002. Apply optimization checklist
...
This step can be the first step if necessary.
After this step, continue to any of Steps `N003` if necessary.

#### Step N003. Generate optimized Dockerfile
...
After this step, continue to any of Steps `N004` if necessary.
...
#### Step N006. Apply compose best practices
...
This step can be the first step if necessary.
After this step, continue to any of Steps `N007` if necessary.

#### Step N007. Generate compose file

First run `echo "[AgentSanitizer] M_N007_begin"`.
...
On completion of this node, run `echo "[AgentSanitizer] M_N007_end"`.
After this step, the skill may stop if necessary.

### `/docker:security` — Container Security Audit

#### Step N008. Dockerfile audit

First run `echo "[AgentSanitizer] M_N008_begin"`.
...
On completion of this node, run `echo "[AgentSanitizer] M_N008_end"`.
This step can be the first step if necessary.
After this step, continue to any of Steps `N009` if necessary.

#### Step N009. Runtime security checks

First run `echo "[AgentSanitizer] M_N009_begin"`.
...
On completion of this node, run `echo "[AgentSanitizer] M_N009_end"`.
After this step, continue to any of Steps `N010` if necessary.

#### Step N010. Generate security report

First run `echo "[AgentSanitizer] M_N010_begin"`.
...
On completion of this node, run `echo "[AgentSanitizer] M_N010_end"`.
After this step, the skill may stop if necessary.
